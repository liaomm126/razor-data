...
class Productanalyzemodel extends CI_Model
{
    ...
    function getTodayInfo($productId, $date)
    {
        ...
        return $query;
         ...
         // "select ifnull(sessions,0) sessions,
         //    ifnull(startusers,0) startusers,
         //        ifnull(newusers,0) newusers,
         //        ifnull(upgradeusers,0) upgradeusers,
         //        ifnull(usingtime,0) usingtime,
         //        ifnull(allusers,0) allusers,
         //        ifnull(allsessions,0) allsessions
         //        from (select date_sk from t_dim_date
         //        where datevalue='2016-06-20') d
         //        left join (select max(dd.date_sk) date_sk,max(allusers) allusers,max(allsessions) allsessions
         //        from t_sum_basic_product bp,
         //        t_dim_date dd where dd.datevalue='2016-06-20' and bp.product_id=1 and bp.date_sk<=dd.date_sk) dpp
         //        on dpp.date_sk=d.date_sk left join(select p.date_sk,sessions,startusers,newusers,upgradeusers,usingtime
         //        from t_sum_basic_product p,
         //        t_dim_date dd where dd.date_sk=p.date_sk and dd.datevalue='2016-06-20' and p.product_id=1) pp
         //    on d.date_sk=pp.date_sk" 
    }
