...
class newusermodel extends CI_Model
{
    ...
    */
    ...
    function getallUserData($fromTime,$toTime)
    {
        ...
        $productId= $currentProduct->id;        //获取当前应用的信息

        $dwdb = $this->load->database('dw', true);  //加载自定义数据库
        $sql = "
            select
                d.datevalue,
                ifnull(sessions,0) sessions,
    ...
    function getDetailUserData($fromTime,$toTime)
    {
      ...
      //var_dump(  $currentProduct );  得到当前应用信息
        $productId= $currentProduct->id;  //得到应用id
        $dwdb = $this->load->database('dw', true);
        $sql = "
            select
                d.datevalue,
    ...
    function getDetailUserDataByDay($fromTime,$toTime)
    {
        ...
        $query = $this->getDetailUserData($fromTime, $toTime);  //根据时间  查询 返回不同的结果

        
        $activeUserRow = $query->first_row();  


        //  var_dump( $activeUserRow  );  //取出最新的一条  obj 类型
