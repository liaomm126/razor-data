...
class Auth extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->helper(array('form','url'));       //加载表单 url 
        $this->load->library('form_validation');        //加载表单验证
        $this->load->library('security');                //Security Class
        $this->load->library('tank_auth');                          //权限类
        $this->load->model('tank_auth/users', 'users');          //加载user model
        $this->lang->load('tank_auth');                         //语言包
        $this->load->library('session');                //开启session
        $this->load->config('email', true);         //邮箱
        $this->load->helper('language');            //加载语言包
    }
    ...
    function login()
    {
        ...
        if ($this->tank_auth->is_logged_in())   //  返回false  , 判断是否为登录状态
        { 
    ...
    function activate()
    {
        ...
        // Activate user
        if ($this->tank_auth->activate_user($user_id, $new_email_key)) { // success $this->tank_auth->logout();
    ...
    function _create_captcha()
    {
        ...
        $cap = create_captcha(
            array('img_path' => './' .
                 $this->config->item('captcha_path', 'tank_auth'),'img_url' => base_url() .
                 $this->config->item('captcha_path', 'tank_auth'),'font_path' => './' .
                 $this->config->item('captcha_fonts_path', 'tank_auth'),'font_size' => 
                 $this->config->item('captcha_font_size', 'tank_auth'),'img_width' => 
                 $this->config->item('captcha_width', 'tank_auth'),'img_height' => 
                 $this->config->item('captcha_height', 'tank_auth'),'show_grid' =>
                 $this->config->item('captcha_grid', 'tank_auth'),'expiration' => $this->config->item('captcha_expire', 'tank_auth'))
        );
    ...
    function _check_captcha($code)
    {
        ...
        if ($now - $time > $this->config->item('captcha_expire', 'tank_auth')) {
            ...
            return false;
                 ...
                 $code != $word) or strtolower($code) != strtolower($word)) {
    ...
    function loginucenter()
    {
        ...
        if ($userKeys) {
            ...
            // //login ucenter testcode : http://dev.cobub.com/users/en/index.php?/auth/login
