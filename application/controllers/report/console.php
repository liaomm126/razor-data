...
class Console extends CI_Controller
{
     ...
     * @return json
     */
    function changeTimePhase($phase = '7day', $fromTime = '', $toTime = '')     //切换时间筛选
    {
       //  var_dump($phase);  //默认当前  筛选时间
       //  var_dump($fromTime);  //自定义筛选的 开始时间
        // var_dump($toTime);     //自定义筛选的 结束时间
    
        $this->common->changeTimeSegment($phase, $fromTime, $toTime);  //时间的
        $ret = array();
        $ret["msg"] = "ok";
...
// 取出 统计实时新增数据 全部日期时间
 ...
 function changeTimeswitch($phase = 'today', $fromTime = '', $toTime = '')     //切换时间筛选
    {
       // var_dump($phase);  //默认当前  筛选时间
       // var_dump($fromTime);  //自定义筛选的 开始时间
       // var_dump($toTime);     //自定义筛选的 结束时间
    
        $this-> common ->changeTimeAll($phase, $fromTime, $toTime);  //时间的
        $ret = array();
        $ret["msg"] = "ok";
        echo json_encode($ret);  
    }
