    ...
    <?php
        ...
        <?php echo  lang('v_rpt_ur_retention')?></h3>
            <div class='submit_link'>
                <select id="selectchannel" name="selectchannel"
                    onchange='changeversionorchannel(selectversion.value, selectchannel.value)'>

                    <option value="all" selected><?php echo lang('v_rpt_el_allChannel'); ?></opton>            
            <?php

if (isset($product_channels)) {
...
?>
        ...
        </select> <select id="selectversion" name="selectversion"
                    onchange='changeversionorchannel(selectversion.value, selectchannel.value)'>
...
// 日留存率
   ...
   console.log('--dayuserdata-',dayuserdata);
    console.log('--weekuserdata-',weekuserdata);
    console.log('--monthuserdata-',weekuserdata);
    ...
    //console.log('--dayuserdata.length--',dayuserdata.length);  1 
  ...
  //  console.log('--pagenum--',pagenum);  1
    ...
    { 
        ...
        var showtime = start;  //开始时间 startdate
        daytr = daytr+"<tr><td>";  
        daytr = daytr + showtime;
        daytr = daytr + "</td><td>";
        daytr = daytr + dayuserdata[i+index].usercount;       //新增用户      
        daytr = daytr + "</td><td>";
...
{    
     // console.log('--myurl--',myurl);  请求地址 返回 json

     var chart_canvas = $('#contents');  //获取对象
    ...
    // 最先加载 loading_img 数据
        ...
        //请求 获取的数据          返回对象中 三个数据对象
        
        jQuery.getJSON(myurl, null, function(data) {

        dayobj=data.userremainday;   // 日 数据
        weekobj=data.userremainweek; // 周 数据
        monthobj=data.userremainmonth;  //月 数据
