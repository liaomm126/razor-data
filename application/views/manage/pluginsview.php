...
<h4 class="alert_warning" id="msg"
		style="color: #BB6616; text-indent: 32px; font-size: 14px; line-height: 28px;">
		<style type="text/css">
a:hover {
...
foreach ($myPlugins as $plugin) {
    ...
    <?php endif; ?>
    <tr>
							<td><p style="font-weight: bold; font-size: 14px;"><?php echo $plugin['name']?></p>
								<br />&nbsp<?php
    ...
    if ($plugin['status'] == 0) {
        ...
        if (isset($guest_roleid) && $guest_roleid == 2) {
                    ?>
                <a style="color: green"><?php echo lang('v_plugins_active')?></a>
       <?php
        } else {
                    ?>
            <a style="color: green"
      ...
      <?php
        ...
        if (isset($guest_roleid) && $guest_roleid == 2) {
                    ?>
            <a style="color: red"><?php echo lang('v_plugins_forbidden')?></a>
      <?php
        } else {
                    ?>
            <a style="color: red"
      ...
      <?php
            ...
            ?>
							...
							<td><p style="font: 14px arial, sans-serif;"><?php echo $plugin['description']?></p>
								<br /><?php echo lang('v_plugins_version')?><?php echo $plugin['version']?>&nbsp &nbsp|&nbsp &nbsp<?php echo lang('v_plugins_provider')?>&nbsp<a
								href="<?php echo $plugin['provider_url'] ?>" target="_blank"><?php echo $plugin['provider']?></a>&nbsp
								&nbsp |&nbsp &nbsp<a href="<?php echo $plugin['detail']?>"
								target="_blank"><?php echo lang('plg_use_instruct')?></a>&nbsp &nbsp|&nbsp &nbsp<?php echo $plugin['date']?></td>
						</tr>
      <?php
}
        ?>
     <?php endif;?>
      ...
      <tr>
							...
							<td><p style="font: 14px arial, sans-serif;"><?php echo $row->plugin_describe?></p>
								<br /><?php echo lang('v_plugins_version')?><?php echo $row->plugin_version?>&nbsp &nbsp|&nbsp &nbsp<?php echo lang('v_plugins_provider')?>&nbsp<a
								href="<?php echo $row->plugin_provider_url;?>" target="_blank"><?php echo $row->plugin_provider?></a>&nbsp
								&nbsp|&nbsp &nbsp<a href="<?php echo $row->plugin_detail?>"
								target="_blank"><?php echo lang('plg_use_instruct')?></a>&nbsp &nbsp|&nbsp &nbsp<?php echo $row->plugin_upload_date?><?php if (isset($row->plugin_sdk)):?>&nbsp &nbsp|&nbsp &nbsp<?php endif;?><?php echo  $row->plugin_sdk ?></td>
						</tr>
      <?php
}
        ?>
