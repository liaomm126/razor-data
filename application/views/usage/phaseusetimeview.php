...
function dealgettime(timephase,fromCurTime,toCurTime)  
{
	...
	totime=toCurTime;
		...
		console.log('-realtime-',realtime);  // today
		console.log('-fromCurTime-',fromCurTime);  // null
		console.log('-toCurTime-',toCurTime);	//null
	...
	if(timephase=='any')
	{		
		myurl="<?php echo site_url()?>/report/productbasic/getTypeAnalyzeData/"+timephase+"/"+fromCurTime+"/"+toCurTime;  // 任意时间段 走这里
	}
	else
	{
		myurl = "<?php echo site_url()?>/report/productbasic/getTypeAnalyzeData/"+timephase+"?date="+new Date().getTime();    // 今天 昨天  7天 30天 调用这里
	} 
...
// 打开页面运行  dealgettime  方法 默认调用 今天数据  
